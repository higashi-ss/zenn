...
※作業用コンテナ（my_app）は、Pythonのアプリを開発する環境です。  
...
* `depends_on`と`healthcheck`で起動順序を制御
  * `my_app`はLocalStackを利用する前提で動くため、`depends_on`に`condition: service_healthy`を指定してLocalStackの準備完了を待ってから起動させます。これにより、起動直後の接続エラーを防ぎます。
